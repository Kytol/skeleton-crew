# 🎃 Goblin Realm - skeleton crew - submission for kiroween 👻

> *Where goblins lurk and tasks go bump in the night...*

An Angular monorepo haunted by mischievous creatures and spooky productivity.
Spawn new web apps instantly using a rich set of reusable UI components.
Lean, fast, and enchantingly easy to build with.

## 🦇 Projects

| App | Description |
|-----|-------------|
| 🧌 **Goblin Tasks** | Task management with goblin minions, XP, loot & dark/light themes |
| ⚔️ **Raid Squad** | Mercenary recruitment portal with squads, missions & treasury |
| 📦 **UI-Lib** | Shared component library (buttons, inputs, calendar) |

**[Demo]( https://kytol.github.io/ui-demo)**

## 🧰 Prerequisites

Before summoning the goblins, make sure you have these installed:

1. Node.js (v18 or later)

Download from: https://nodejs.org/

Check your version:
```bash
node -v
```
2. npm (comes with Node)

Check version:
```bash
npm -v
```
3. Angular CLI

Install globally:
```bash
npm install -g @angular/cli
```
## 🕸️ Quick Start

```bash
npm install
npm run start -- --project=goblin-tasks   # 🧌
npm run start -- --project=raid-squad     # ⚔️
```

## 🪦 Features

- 🌙 Dark/Light theme toggle
- 🎲 Mini-games & gambling
- 🏆 Achievements & leveling
- 💰 Economy & shop system
- 📜 Quest chains
- 🌤️ Dynamic weather effects

## 🔮 Tech Stack

Angular 18 • TypeScript • Signals • Standalone Components

---

📜 License

This project is open source under the MIT License.
👉 **[LICENSE](https://github.com/Kytol/skeleton-crew/blob/main/LICENSE)**

---

*Built with 👻 Kiro and a sprinkle of goblin magic 🧌*
